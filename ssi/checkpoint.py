...
class FullModelTorchTuneCheckpointer(_CheckpointerInterface):
    ...
    def load_checkpoint(self, weights_only: bool = True) -> Dict[str, Any]:
        ...
        state_dict[training.MODEL_KEY] = safe_torch_load(self._checkpoint_path, weights_only=weights_only)
    ...
    def save_checkpoint(
        ...
        if not adapter_only:
            shard_name = SHARD_FNAME.format(cpt_idx="1".zfill(5), num_shards="1".zfill(5))
            output_path = Path.joinpath(self._output_dir, f"epoch_{epoch}", shard_name).with_suffix(".bin")
...
class FullModelMetaCheckpointer(_CheckpointerInterface):
    ...
    def save_checkpoint(
        ...
        if not adapter_only:
            ...
            if self._model_type == ModelType.LLAMA3_VISION:
                from torchtune.models.llama3_2_vision._convert_weights import llama3_vision_tune_to_meta
            ...
            model_filename = SHARD_FNAME.format(cpt_idx="1".zfill(5), num_shards="1".zfill(5))
            checkpoint_file = Path.joinpath(self._output_dir, f"epoch_{epoch}", model_filename).with_suffix(".bin")
