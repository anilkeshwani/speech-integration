...
Tests use n_samples config to stream only small subsets (near-zero disk).
    ...
    with initialize_config_dir(config_dir=abs_conf_dir, version_base=None):
        cfg = compose(
            ...
            overrides=[
                ...
                f"data.train.dataset.n_samples={train_n}",
                f"data.dev.dataset.n_samples={dev_n}",
                "log_interval=1",
...
def _setup_trainer(cfg) -> Trainer:
    """Create and set up a Trainer. Data subsetting handled via n_samples config."""
    trainer = Trainer(cfg)
    trainer.setup()
    return trainer
...
def test_evaluate_returns_finite(tmp_path):
    ...
    cfg = _compose_sft_cfg(tmp_path, dev_n=20)
    trainer = _setup_trainer(cfg)
